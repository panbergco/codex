...
- `fs/uploadFile` accepts a client-visible `fileName`, stores only its final path component, chooses the destination under `$CODEX_HOME/uploads/<uuid>/`, and rejects files larger than 45 MiB so a base64 JSON request stays below the current direct WebSocket message ceiling while clients avoid arbitrary host paths or unbounded payloads.
